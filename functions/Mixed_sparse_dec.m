function [X, A, B, S, D, Out] = Mixed_sparse_dec(Y, opts,WstrucTen)
...
for k=1: max_it
    ...
    S = prox_l1(WstrucTen.*(Y-X+rho*Sk)/(1+rho),W.*mu/(1+rho));
...
for i=1:Bmax_it
    ...
    %% Z subproblem
     ...
     W1 = 1/(abs(double(ttm(tensor(B),D{1},1))+P1/beta)+eps);
    Z1 = prox_l1(double(ttm(tensor(B),D{1},1))+P1/beta,W1 *tau/beta);
